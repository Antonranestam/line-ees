...
const RandomPathGenerator = () => {
  ...
  const generateFluidPath = () => {
    ...
    const geometry = new THREE.TubeGeometry(curve, CURVE_POINTS, LINE_WIDTH / 2, 8, false);
    const material = createGradientMaterial(colors);

    const newPath = new THREE.Mesh(geometry, material);
    newPath.scale.set(0.9, 0.9 * A4_RATIO, 1); // Scale to fit A4 ratio
    scene.add(newPath);
